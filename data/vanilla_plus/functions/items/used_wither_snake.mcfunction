...
execute if entity @s[nbt={SelectedItem:{tag:{CustomModelData:20210224}}}] as @e[type=arrow,nbt={inGround:0b},distance=..5] unless data entity @s Potion unless data entity @s {pickup:0b} run data merge entity @s {Color:328965,CustomPotionEffects:[{Id:20b,Amplifier:1b,Duration:100}],Potion:"minecraft:empty"}
